import Row from 'react-bootstrap/Row'
import Col from 'react-bootstrap/Col'
import Card from 'react-bootstrap/Card'
import Button from 'react-bootstrap/Button'
import CardGroup from 'react-bootstrap/CardGroup'

import LandingHeader from '../components/LandingHeader'

const LandingPage = () => {
	return (
		<>
			<LandingHeader />
			<div>
				<Row className='showcaseContainer m-0 p-0'>
					<Col md={0} lg={3}>
						<img
							src='https://cdn.douglasavenue.com/krk/media/img/krk-logo.png'
							alt='KRK Logo'
							className='img-fluid'
						/>
					</Col>
					<Col md={12} lg={9}>
						<h2>Welcome To KRK!</h2>
						<p>
							KRK (pronounced 'cork') EBBS system is a digital
							corkboard for businesses that allows employees to
							communicate with each other, management to
							communicate with all employees, and event
							scheduling. Check out the information below to see
							more details on how KRK can help your business!
						</p>
					</Col>
				</Row>
			</div>

			<CardGroup className='landingCards'>
				<Col md={12} lg={4}>
					<Card className='mt-2 h-100 landingCard'>
						<Card.Img
							variant='top'
							src='https://cdn.douglasavenue.com/krk/media/img/post-sales-header.png'
						/>
						<Card.Body>
							<Card.Title className='card-title'>
								Post Feed For Users!
							</Card.Title>
							<Card.Text>
								Users can post messages, as well as like and
								reply to messages other users’ post. Messages
								are segregated to departments, so it is as if
								each department has their own personal KRKBoard!
								Supervisors for that department can prioritize
								messages so they remain at the top of the
								message timeline, and site-wide supervisors can
								send messages to any department as well as pin
								sitewide messages to the top of the timeline!
							</Card.Text>
							<Card.Text>
								Feel free to log in with our test user account
								or register with a new account to test KRK out,
								or contact me, Erin Winking, to discuss the
								application further!
							</Card.Text>
						</Card.Body>
						<Card.Footer>
							<Row>
								<Button
									variant='success'
									href='/login'
									className='my-2'>
									LOGIN/REGISTER
								</Button>
								<Button
									variant='primary'
									href='https://imageone.digital/contact/'
									className='my-2'>
									CONTACT ERIN
								</Button>
							</Row>
						</Card.Footer>
					</Card>
				</Col>

				<Col md={12} lg={4}>
					<Card className='mt-2 h-100 landingCard'>
						<Card.Img
							src='https://cdn.douglasavenue.com/krk/media/img/event-sales-header.png'
							alt='Events!'
						/>
						<Card.Body>
							<Card.Title className='card-title'>
								Event Calendar For All!
							</Card.Title>
							<Card.Text>
								KRK comes with an event calendar for your staff!
								Events can be created as far in advance as you
								like and can be set to only be displayed for a
								single department or the entire site! Sometimes
								your users do not need to know about an event or
								project happening in another department, so no
								need to clutter up their event calendar.
							</Card.Text>
							<br />
							<Card.Text>
								Feel free to log in with our test user account
								or register with a new account to test KRK out,
								or contact me, Erin Winking, to discuss the
								application further!
							</Card.Text>
						</Card.Body>
						<Card.Footer>
							<Row>
								<Button
									href='/login'
									variant='success'
									className='my-2'>
									LOGIN/REGISTER
								</Button>
								<Button
									href='https://imageone.digital/contact/'
									variant='primary'
									className='my-2'>
									CONTACT ERIN
								</Button>
							</Row>
						</Card.Footer>
					</Card>
				</Col>
				<Col md={12} lg={4}>
					<Card className='mt-2 h-100 landingCard'>
						<Card.Img
							src='https://cdn.douglasavenue.com/krk/media/img/user-sales-header.png'
							alt='Users!'
						/>
						<Card.Body>
							<Card.Title>Easy User Management!</Card.Title>
							<Card.Text>
								User management is KRK is simple. Users register
								their account, supervisors set their department
								as well as their access levels, and that is it!
								Users can upload their own profile pictures and
								set their own biographies. Supervisors and
								managers can of course edit anything necessary!
							</Card.Text>
							<br />
							<Card.Text>
								Feel free to log in with our test user account
								or register with a new account to test KRK out,
								or contact me, Erin Winking, to discuss the
								application further!
							</Card.Text>
						</Card.Body>
						<Card.Footer>
							<Row>
								<Button
									href='/login'
									variant='success'
									className='my-2'>
									LOGIN/REGISTER
								</Button>
								<Button
									href='https://imageone.digital/contact/'
									variant='primary'
									className='my-2'>
									CONTACT ERIN
								</Button>
							</Row>
						</Card.Footer>
					</Card>
				</Col>
			</CardGroup>
			<CardGroup className='landingCards'>
				<Card md={12} lg={6} className='landingCard'>
					<Card.Body>
						<Card.Title>Try KRK Out!</Card.Title>
						<Card.Text>
							Would you like to take KRK for a spin? Please try
							out the demo site by clicking the buttons below or
							the buttons on the top menu! You can either create
							your own account, which will default to give you the
							view of a front-line worker at our fictional
							restaurant, MgRonald's. If you don't want to go
							through all that trouble, you can use the demo
							account that is already set up. User name is
							'testuser' and the password is 'testpassword'
							(without the quotes).
						</Card.Text>
					</Card.Body>
					<Card.Footer>
						<Row>
							<Button
								href='/login'
								variant='secondary'
								className='my-2'>
								LOGIN
							</Button>
							<Button
								href='/register'
								variant='success'
								className='my-2'>
								REGISTER NEW ACCOUNT
							</Button>
						</Row>
					</Card.Footer>
				</Card>
				<Card md={12} lg={6} className='landingCard'>
					<Card.Body>
						<Card.Title>KRK For Your Biz!</Card.Title>
						<Card.Text>
							Would you like to have your own KRK Employee
							Bulletin Board System? Well, then you will have to
							hire me as a web developer and have me build it for
							you! KRK isn't sold as SaaS right now (but who knows
							what the future might hold). You can look into
							having me, Erin Winking, as an employee by checking
							out my profile, GitHub, or emailing me!
						</Card.Text>
					</Card.Body>
					<Card.Footer>
						<Row>
							<Button
								href='https://douglasavenue.com'
								variant='secondary'
								className='my-2'>
								MY PROFILE
							</Button>
							<Button
								href='https://github.com/ewink1977'
								variant='secondary'
								className='my-2'>
								MY GITHUB
							</Button>
							<Button
								href='https://imageone.digital/contact/'
								variant='warning'
								className='my-2'>
								ERIN@DOUGLASAVENUE.COM
							</Button>
						</Row>
					</Card.Footer>
				</Card>
			</CardGroup>
		</>
	)
}

export default LandingPage
